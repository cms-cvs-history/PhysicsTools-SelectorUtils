...
class MuonVPlusJetsIDSelectionFunctor : public Selector<pat::Muon> {
 ...
 public: // interface
  ...
  MuonVPlusJetsIDSelectionFunctor( edm::ParameterSet const & parameters ) {
    ...
    else if ( versionStr == "SPRING11" ) {
      version = SPRING11;
      if (verbose_) std::cout << "\nMUON SELECTION - you are using SPRING11 Selection" << std::endl;
    }
  ...
  { 
    ...
    if (version_ == SPRING10 || version_ == FALL10 || version_ == SPRING11 ) throw cms::Exception("LogicError") 
      << "MuonVPlusJetsSelectionFunctor SPRING10, FALL10 and SPRING11 versions needs the event! Call operator()(muon,event,ret)"
      <<std::endl;
